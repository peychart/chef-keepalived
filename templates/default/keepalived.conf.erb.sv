...
vrrp_instance <%= name.upcase %> {
  ...
  <% end -%>
  virtual_ipaddress {
    <% Array(instance['ip_addresses'][node.domain] || instance['ip_addresses']).each do |address| %>
    <%= address %>
    <% end if instance['ip_addresses'] %>
  }
#  <% (instance['real_servers'][node.domain] || instance['real_servers']).each do |virtual_server| %>
#  virtual_server <%= virtual_server['bind'][node.domain] || virtual_server['bind'] %> {
#    <% ((virtual_server['options'].is_a? Array) ? virtual_server['options'] : Array[virtual_server['options']]).each do |option| %>
#      <%= option %>
#    <% end %>
#    <% virtual_server['real_servers'][node.domain].each do |real_server| %>
#    real_server <%= real_server['bind'][node.domain] || real_server['bind'] %> {
#      <% (real_server['bind'][node.domain] || real_server['bind']).each do |name, value| %>
#        <%= "#{name} {" if name[-1] != '-' %>
